...
def downloadCalendar():
  ...
  psw = password
  can_login(user, psw, session, site)
...
def main():
  lessons = downloadCalendar()
  print_lessons(lessons) if len(lessons) > 0 else print(
    colored(f"No lessons found", "yellow", attrs=["underline"]))
