...
def extract_info(info):
    ...
    for _lesson in info:
        ...
        if lesson["isDone"] == True:
            ...
            datetime.today() < lesson_date_end and 
...
def print_lessons(lessons):
    ...
    for l in lessons:
        ...
        # avoid lesson's day is printed twice (one for the morning, and one for the afternoon)
...
# only_current_day: if print only the current day
def bot_print(lessons, id):
    ...
    for l in lessons:
        ...
        materia = f"\n📓 | {subject}"
        stanza = f"\n🏢 | {room}"
        stringa = data + orario + docente + materia + stanza

        if type_ == "esame":
            tipo = "\n\n⚠️ *ESAME* ⚠️\n\n"
            stringa = tipo + stringa + tipo
...
def checkDB(oldDB):
    ...
    if oldDB != newDB:
        oldDB = newDB
        global day
        day = requestGeop(date.today(), date.today()+timedelta(days=1))
        # newsletter()
